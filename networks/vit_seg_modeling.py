...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        self.hybrid = None
        self.config = config
        img_size = _pair(img_size)  # (224, 244)
        ...
        if self.hybrid:
            self.hybrid_model = ResNetV2(block_units=config.resnet.num_layers, width_factor=config.resnet.width_factor)  # ResNet, 3 blocks
            in_channels = self.hybrid_model.width * 16  # 64
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       out_channels=config.hidden_size,
                                       kernel_size=patch_size,
                                       stride=patch_size)  # 1024, 768, (1,1), (1,1)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis):
        ...
        self.embeddings = Embeddings(config, img_size=img_size) # CNN and patch embedding
        self.encoder = Encoder(config, vis)  # transformer encoder
...
class VisionTransformer_mixRf(nn.Module):
    def __init__(self, config, img_size=224, num_classes=2, zero_head=False, vis=False):
        ...
        self.transformer_rf = None
        self.rf_stn = TPSTransformer()  # for RF data
    ...
    def forward(self, x, rf=None):
        ...
        if x.size()[1] == 1:
            if rf is not None:
                rf = self.rf_stn(rf)
                x = torch.cat([x+rf, x+rf, x+rf], dim=1)  # (B, 3, H, W)
            else:
                x = x.repeat(1, 3, 1, 1)
    ...
    def load_from(self, weights):
        ...
        if self.transformer_rf is not None:
            with torch.no_grad():
